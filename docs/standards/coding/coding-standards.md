...
For Python code style, the conventions outlined in [PEP 8 — Style Guide for Python Code](https://peps.python.org/pep-0008/) will be followed. This document supplements PEP 8 with additional guidelines specific to the AI Booster Plus project.
...
- `master` is the release branch and must remain production-ready.

Branching and merging rules:

- `master` and `development` are protected. Direct pushes are not allowed; changes must be merged via merge request (MR).
...
- Promotion to `master` occurs via an MR from `development` to `master` when a release is ready.
- Releases are cut from `master` using tags (e.g., `vX.Y.Z`).
- Hotfixes should branch from `master` (e.g., `hotfix/<topic>`) and be merged back into both `master` and `development`.
...
Commit messages must be clear and descriptive, following the [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) format:

`type(scope): subject`

Use standard Conventional Commit types such as `feat`, `fix`, `refactor`, `docs`, and `chore`.

Non-trivial commits must include a body — a blank line after the subject line, followed by a bullet list of concrete changes made:

```text
feat(auth): add JWT-based login endpoint

- Add POST /auth/login endpoint in src/api/auth.py
- Add JWTService.encode() and JWTService.decode() in src/services/jwt.py
- Add unit tests for login success, invalid credentials, and token expiry
- Update requirements.txt with PyJWT>=2.8
```

Body guidelines:

- One bullet per logical change; keep each line concise (fits in ~80 chars).
- Use present-tense imperative ("Add …", "Remove …", "Fix …", "Update …").
- Omit the body only for genuinely single-action commits (e.g. fixing a typo).
- Never include AI/tool co-authorship attribution in commit messages.

Additional source control guidelines:

- Use dedicated branches for new features, refactors, or bug fixes. Branch names should reflect the feature or issue being addressed (e.g. `feat/auth-login`, `fix/model-serialization`).
- Regularly pull or rebase from the target branch (`development` for feature work) to keep your branch up to date and minimise merge conflicts.
- Before merging, ensure that all automated tests pass, the code adheres to these standards, and the MR has been reviewed and approved.
...
The AI Booster Plus project is organised as follows on GitLab:

```text
vodacomsa/
└── ai-booster-plus/                    # Main project group
    ├── aib-genai-standards/            # This repo
...
- [GitLab — Merge request best practices](https://docs.gitlab.com/ee/user/project/merge_requests/reviews/)
